class ArtistProblematic {
    ...
    async getData(params) {
        const url = '/v1/stats/artist-problematic?' + new URLSearchParams(params).toString();
    ...
    generateTable(data) {
        ...
        const rows = data
            ...
            .map(item => {
                const searchQuery = encodeURIComponent(item.artist_name || '');
            ...
            `;
                ...
                const tidalCell = item.tidal_url ?
                    `<a href="${item.tidal_url}" target="_blank" style="font-size: 10px;">${item.tidal_url}</a> 
                        <span onclick="pageController.artistProblematic.clearUrl(${item.id}, 'tidal')" 
                                style="color: red; cursor: pointer; font-size: 12px; margin-left: 5px;" 
                                title="Очистить URL">&times;</span>` :
                    `<input type="text" placeholder="Tidal URL" style="width:100px;font-size:10px;" onchange="pageController.artistProblematic.updateUrl(${item.id}, 'tidal', this.value)">`;

                const spotifyCell = item.spotify_url ?
                    `<a href="${item.spotify_url}" target="_blank" style="font-size: 10px;">${item.spotify_url}</a> 
                        <span onclick="pageController.artistProblematic.clearUrl(${item.id}, 'spotify')" 
                            style="color: red; cursor: pointer; font-size: 12px; margin-left: 5px;" 
                            title="Очистить URL">&times;</span>` :
                    `<input type="text" placeholder="Spotify URL" style="width:100px;font-size:10px;" onchange="pageController.artistProblematic.updateUrl(${item.id}, 'spotify', this.value)">`;

                const statusButtons = `
                <div style="display: flex; flex-direction: column; gap: 2px; font-size: 10px;">
                    <button onclick="pageController.artistProblematic.updateStatus(${item.artist_id}, 0)" style="padding: 1px 3px; font-size: 9px;" ${item.status == '0' ? 'disabled' : ''}>На переобход</button>
                    <button onclick="pageController.artistProblematic.updateStatus(${item.artist_id}, 1)" style="padding: 1px 3px; font-size: 9px;" ${item.status == '1' ? 'disabled' : ''}>Без isrc</button>
                    <button onclick="pageController.artistProblematic.updateStatus(${item.artist_id}, 2)" style="padding: 1px 3px; font-size: 9px;" ${item.status == '2' ? 'disabled' : ''}>С isrc</button>
                    <button onclick="pageController.artistProblematic.updateStatus(${item.artist_id}, 3)" style="padding: 1px 3px; font-size: 9px;" ${item.status == '3' ? 'disabled' : ''}>Отсутсвует Tidal</button>
                    <button onclick="pageController.artistProblematic.updateStatus(${item.artist_id}, 4)" style="padding: 1px 3px; font-size: 9px;" ${item.status == '4' ? 'disabled' : ''}>Нет ссылок</button>
                    <button onclick="pageController.artistProblematic.updateStatus(${item.artist_id}, 5)" style="padding: 1px 3px; font-size: 9px;" ${item.status == '5' ? 'disabled' : ''}>В работе</button>
                </div>
            `;

                return `
                <tr data-artist-id="${item.id}">
                    <td><input type="checkbox" class="artist-checkbox" value="${item.artist_id}"></td>
                    <td>${item.id || ''}</td>
                    ...
                    <td style="text-align: center; font-size: 22px;">
                        <span class="clicking" onclick="pageController.artistProblematic.getSocials(${item.artist_id || 0}, decodeURIComponent('${encodeURIComponent(item.artist_name || '')}'), decodeURIComponent('${encodeURIComponent(item.tidal_url || '')}'), decodeURIComponent('${encodeURIComponent('problematic' || '')}'))">⚙️</span>
                    </td>
                </tr>
                `;
            }).join('');
    ...
    async getSocials(artistId, artistName, tidalUrl = '', trackName = 'ArtistProblematic') {
    ...
    showSocialsModal(artistId, artistName, socials, tidalUrl = '', trackName = 'ArtistProblematic') {
        ...
        <div style="background-color: #5969fc24; margin: -8px -8px 8px -8px; padding: 8px;">
            <table class="table-mini">
                ...
                <td><label>&nbsp;</label><br><button onclick="pageController.artistProblematic.addSocial(${artistId})">add</button></td>
            </table>
        </div>
    `;
        ...
        socials.forEach(social => {
            ...
            <div style="border: 1px solid #5969fc24; border-radius: 8px; padding: 8px; margin: 8px 0;">
                <table style="width: 100%;">
                    ...
                    <td><div style="float: right; cursor: pointer;" onclick="pageController.artistProblematic.deleteSocial(${artistId}, ${social.id})">🗑️</div></td>
                </table>
                <table class="table-mini" style="width: 100%;">
                    ...
                    <td><label>&nbsp;</label><br><button onclick="pageController.artistProblematic.editSocial(${artistId}, ${social.id})">save</button></td>
                </table>
            </div>
        `;
        });
    ...
    async updateStatus(artistId, newStatus) {
        try {
            loader(1);

            const response = await fetch(`/v1/stats/artist-problematic/status`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ artistId: artistId, status: newStatus })
            });
            ...
            alert('Не удалось обновить статус артиста');
        } finally {
            loader(0);
        }
    }

    async updateUrl(artistId, platform, url) {
        if (!url.trim()) return;
        ...
        const response = await fetch(`/v1/stats/artist-problematic/${artistId}/url`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        if (response.ok) {
            const tr = document.querySelector(`tr[data-artist-id="${artistId}"]`);
            if (tr) {
                const tds = tr.querySelectorAll('td');
                const platformIndex = platform === 'tidal' ? 5 : 6;
                const td = tds[platformIndex];
                if (td) {
                    const linkHtml = `<a href="${url}" target="_blank" style="font-size: 10px;">${url}</a> 
                                  <span onclick="pageController.artistProblematic.clearUrl(${artistId}, '${platform}')" 
                                        style="color: red; cursor: pointer; font-size: 12px; margin-left: 5px;" 
                                        title="Очистить URL">&times;</span>`;
    ...
    async clearUrl(artistId, platform) {
        const body = {};
        if (platform === 'tidal') {
            body.tidal_url = '';
        } else if (platform === 'spotify') {
            ...
            if (tr) {
                ...
                if (td) {
                    const inputHtml = platform === 'tidal'
                        ? `<input type="text" placeholder="Tidal URL" style="width:100px;font-size:10px;" onchange="pageController.artistProblematic.updateUrl(${artistId}, 'tidal', this.value)">`
                        : `<input type="text" placeholder="Spotify URL" style="width:100px;font-size:10px;" onchange="pageController.artistProblematic.updateUrl(${artistId}, 'spotify', this.value)">`;
    ...
    async bulkUpdate() {
        ...
        try {
            ...
            const response = await fetch('/v1/stats/artist-problematic/status', {
                ...
                body: JSON.stringify({ artistId: ids, status: status })
            });
