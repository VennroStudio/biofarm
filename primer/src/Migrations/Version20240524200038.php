<?php

declare(strict_types=1);

namespace App\Migrations;

use Doctrine\DBAL\Schema\Schema;
use Doctrine\Migrations\AbstractMigration;

/**
 * Auto-generated Migration: Please modify to your needs!
 */
final class Version20240524200038 extends AbstractMigration
{
    public function getDescription(): string
    {
        return '';
    }

    public function up(Schema $schema): void
    {
        // this up() migration is auto-generated, please modify it to your needs
        $this->addSql('DROP INDEX IDX_ARTIST_ID ON album_artists');
        $this->addSql('CREATE INDEX IDX_ARTIST_ID ON album_artists (artist_id, is_loaded)');
        $this->addSql('DROP INDEX IDX_COUNTER ON albums');
        $this->addSql('CREATE INDEX IDX_COUNTER ON albums (all_tracks_mapped, is_approved, lo_album_id)');
    }

    public function down(Schema $schema): void
    {
        // this down() migration is auto-generated, please modify it to your needs
        $this->addSql('DROP INDEX IDX_ARTIST_ID ON album_artists');
        $this->addSql('CREATE INDEX IDX_ARTIST_ID ON album_artists (artist_id)');
        $this->addSql('DROP INDEX IDX_COUNTER ON albums');
        $this->addSql('CREATE INDEX IDX_COUNTER ON albums (is_approved, all_tracks_mapped, lo_album_id)');
    }
}
