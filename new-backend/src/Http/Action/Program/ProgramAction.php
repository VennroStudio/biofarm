    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        try {
            $result = match (true) {
                ...
                $method === 'GET' && \in_array($part, ['team', 'ledger', 'sales', 'withdrawals', 'audit'], true) => $this->program->listing($part, $admin ? null : $actor, (int)($query['page'] ?? 1), (int)($query['limit'] ?? 25), (string)($query['sort'] ?? 'depth'), (string)($query['direction'] ?? 'asc'), isset($query['wallet']) ? (string)$query['wallet'] : null, isset($query['dateFrom']) ? (string)$query['dateFrom'] : null, isset($query['dateTo']) ? (string)$query['dateTo'] : null),
