...
export function PartnerDialog({ onClose, title, children, wide = false }: { onClose: () => void; title: string; children: ReactNode; wide?: boolean }) {
    ...
    return createPortal(
        <dialog ref={ref} aria-labelledby={titleId}
            className={`fixed inset-0 m-auto max-h-[90dvh] w-[calc(100%_-_2rem)] ${wide ? "max-w-3xl" : "max-w-md"} overflow-y-auto rounded-2xl border border-border bg-white p-5 text-foreground shadow-xl backdrop:bg-foreground/45 sm:p-6`}
