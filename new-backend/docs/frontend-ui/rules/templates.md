...
React source повторяет ownership Twig, если блоку нужно UI-поведение:
...
React island используется для интерактивного поведения внутри уже отрендеренного Twig HTML: меню, модалки, анимации, локальное состояние.
Формы не переводить в browser-запросы: они отправляются обычным POST в web-контроллер.
...
Формы внутри widget отправляются обычным POST в web-контроллер; контроллер делает redirect обратно к widget или рендерит страницу с ошибкой.
...
- Twig получает модели и view objects, не сырые массивы из источника данных.
